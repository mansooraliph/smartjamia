...
import { EmptyState, ErrorBanner, ScreenTitle } from '../../src/components/ui';
import { PortalApi, TimetableGrid } from '../../src/lib/portal.api';
...
export default function StudentTimetable() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await PortalApi.timetable();
  ...
  return (
    <ScrollView style={{ backgroundColor: colors.bg }} contentContainerStyle={styles.container}>
      <ScreenTitle subtitle="Your weekly class schedule">Timetable</ScreenTitle>
      <ErrorBanner message={error} />

      {!enrolled || !grid ? (
        <EmptyState message="No active enrollment found for this academic year." />
      ) : (
        <>
              ...
              <View>
                ...
                {grid.periods.map((p) => (
                  <View key={p.periodNumber} style={styles.row}>
                    ...
                    {grid.days.map((d) => {
                      const cell = grid.cells.find((c) => c.dayOfWeek === d && c.periodNumber === p.periodNumber);
                      const tone = cell ? colorForKey(cell.subject) : colors.faint;
                      return (
                        <View key={d} style={styles.dayCell}>
                          {cell ? (
                            <View style={[styles.subjectChip, { backgroundColor: `${tone}1A`, borderColor: `${tone}40` }]}>
                              <Text style={[styles.subjectText, { color: tone }]} numberOfLines={1}>{cell.subject}</Text>
                              {cell.teacher && <Text style={styles.teacherText} numberOfLines={1}>{cell.teacher}</Text>}
                            </View>
                          ) : (
...
const styles = StyleSheet.create({
  ...
  classBadge: { marginBottom: spacing.md },
  className: { fontSize: 14, fontWeight: '800', color: colors.ink },
